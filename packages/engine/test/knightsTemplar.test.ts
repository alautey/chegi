...
function setupCapture(knightHasMoved: boolean, bishopHasMoved = false) {
  const g = emptyGame();
  ...
  put(g, { file: 7, rank: 7 }, 'B', 'b', { hasMoved: bishopHasMoved });
  g.turn = 'b';
...
describe('Knights Templar (interpretation flagged in rules.ts — confirm with the rules author)', () => {
  ...
  it('does not grant the capture if the capturing Bishop had already moved (returned to its home square)', () => {
    const g = setupCapture(false, true);
    const special = g
      .legalMoves()
      .find((m) => m.kind === 'move' && m.from.file === 1 && m.from.rank === 0 && m.to.file === 0 && m.to.rank === 0);
    expect(special).toBeFalsy();
